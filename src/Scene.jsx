...
import AnimatedTitle from './AnimatedTitle'; // Importar el componente de título animado
import Loader from './Loader'; // Importar el nuevo componente Loader

const SceneContent = ({ allowInteraction, setLoading }) => {
  ...
  useEffect(() => {
    ...
    // Crear una esfera iluminada con un material avanzado y un mapa de ruido
    const sphereGeometry = new THREE.SphereGeometry(30, 64, 32); // Reducir el tamaño de la esfera
    const sphereMaterial = new THREE.MeshStandardMaterial({
      color: 0x555555, // Gris medio
      roughness: 0.5,
      metalness: 0.5,
      displacementMap: new THREE.TextureLoader().load('path_to_noise_texture.jpg'),
      displacementScale: 5, // Reducir la escala del desplazamiento
    });
  ...
  }, [camera, scene, gl, allowInteraction, setLoading]);
...
const Scene = ({ allowInteraction }) => {
  ...
  return (
    <>
      ...
      <AnimatedTitle /> {/* Integrar el componente de título animado */}
    </>
  );
};
